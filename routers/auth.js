...
router.post('/register', registerRules, async (req, res, next) => {
  ...
  // 檢查 email 是否已被註冊過
  let [member] = await connection.execute('SELECT * from users WHERE email=?', [
    req.body.email,
  ])
  console.log(member)
  if (member.length > 0) {
  ...
  let [result] = await connection.execute(
    'INSERT INTO users (name, email, password, valid, created_at) VALUES (?, ?, ?, ?, ?)',
    [req.body.name, req.body.email, hashPassword, 1, signUpTime]
  )
